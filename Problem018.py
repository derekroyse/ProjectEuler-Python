...
# --- Values found on https://projecteuler.net/problem=18 ---
...
topRow = triangle[index]
bottomRow = triangle[len(triangle)-1]

while len(bottomRow) >= 3:  
    tempRow = []
    for i in range(len(topRow)):
        tempRow.append(topRow[i] + max(bottomRow[i],bottomRow[i+1]))            
    bottomRow = tempRow
    index-=1
    topRow = triangle[index]

answer = topRow + max(bottomRow[0],bottomRow[1])
print(answer)